<view class="container">
  ...
  <view class="list">
    ...
    <view class="list-item">
      {emptyArray.map((item, index) =>
        <text class="request-text" style={`color: ${step === index + 1 ? 'red' : 'black'}`}>{index + 1}</text>
      )}
    </view>

    {openid && <view class="list-item" >
      <text class="request-text">openid：{openid}</text>
    </view>
    }

    {counterId && <view class="list-item" >
  ...
  {step === 3 && <view class="guide" >
    ...
    {counterId && <text class="p" >新增的记录 _id 为：{counterId}</text>}
    <text class="p">5. 在云开发 -> 数据库 -> counters 集合中可以看到新增的记录</text>

    <div class="nav">
      <button class="prev" size="mini" type="default" bindtap="prevStep">上一步</button>
      {counterId && <button class="next" size="mini" type="default" bindtap="nextStep">下一步</button>}
    </div>
  </view>}


  {/* 查询记录 */}
  {step === 4 && <view class="guide">
    ...
    {queryResult && <text class="code" >{queryResult}</text>}
  ...
  {step === 6 && <view class="guide">
    ...
    <div class="nav">
      {counterId && <button class="prev" size="mini" type="default" bindtap="prevStep">上一步</button>}
      <button class="next" size="mini" type="default" bindtap="nextStep">下一步</button>
    </div>
  </view>}
